...
const getInitialState = () => {
    ...
    try {
        ...
        if (savedIndex) {
            notes = JSON.parse(savedIndex).map((note: any) => note.lastUpdatedAt ? note : { ...note, lastUpdatedAt: note.createdAt });
        }
        ...
        return {
            ...
            notes: [{ id: 'fallback', name: 'Error Note', createdAt: Date.now(), lastUpdatedAt: Date.now() }],
            theme: 'light'
        };
    }
}
  ...
  const handleDeleteNote = (noteIdToDelete: string) => {
    ...
    if (activeNoteId === noteIdToDelete) {
        if (updatedNotes.length > 0) {
            ...
            const sortedNotes = [...updatedNotes].sort((a,b) => b.lastUpdatedAt - a.lastUpdatedAt);
